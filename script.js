let data = [
  ...
  //   var inputData = await data.json();
  console.log('paginations')
  let tableElement = document.getElementById('table_body')
  let pageNumber = document.getElementById('pageNumber')
  
  current_page = 1;
  row = 10;
  
  function displayItem(current_page, row, items, wrapper) {
    //passing the defaule page, number of rows, data ,and html element as argument of function
    wrapper.innerHTML = ''; //the innerelement
    current_page--;
    ...
    let pageItem = items.slice(start, end);
    console.log(pageItem)
    pageItem.map((element) => {
      let items_element = document.createElement('tr');
      items_element.innerHTML = `
      <td>${element.id}</td>
      <td>${element.name}</td>
      <td>${element.email}</td>
      `
      wrapper.append(items_element)
    })
  }
  
  function pagination(items, row, page_No_element) {
    page_No_element.innerHTML = ""; //start the page number is empty
    let pageCount = Math.ceil(items.length / row);
    
    for (let i = 1; i < pageCount + 1; i++) { //for loop for the number of page loops
      let pageNo = pageNoButton(items, i)
      page_No_element.append(pageNo)
    }
  }
  
  function pageNoButton(items, page) {
    let button = document.createElement('li')
    button.classList.add('page-item')
    let button_link = document.createElement('a');
    button_link.classList.add('page-link')
    ...
    button.addEventListener('click', () => {
      current_page = page;
      displayItem(current_page, row, items, tableElement)
  ...
  displayItem(current_page, row, data, tableElement)
  pagination(data, row, pageNumber)